...
> [!NOTE]
> **A newer PHP SDK is available!** We recommend using the new [`dwolla-php`](https://github.com/Dwolla/dwolla-php) SDK for new projects. The new SDK offers improved type safety, better error handling, and a more intuitive API. This legacy SDK (`dwolla-swagger-php`) will continue to be available, but new features and improvements will be focused on the new SDK.
